		...
		public void DoUpdateGraphs () {
			...
			if (!colliderEnabled) {
				...
				if (BoundsVolume(merged) < BoundsVolume(newBounds) + BoundsVolume(prevBounds)) {
                    ...
                    GraphUpdateObject mergedGuo = new GraphUpdateObject(merged);

                    mergedGuo.updatePhysics = false;
                    mergedGuo.modifyTag = true;
                    mergedGuo.setTag = 0;

                    AstarPath.active.UpdateGraphs(mergedGuo);
                } else {
                    ...
                    GraphUpdateObject currentGuo = new GraphUpdateObject(newBounds);

                    currentGuo.updatePhysics = false;
                    currentGuo.modifyTag = true;
                    currentGuo.setTag = objectTag;
